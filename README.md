...
Working and tested, but early. `cargo test` covers 65 cases; `cargo clippy -D warnings`
...
The orchestrator runs the sub-task and returns the result. Sub-agent replies are not
re-scanned for delegations, and at most 3 delegations run per turn, so the swarm cannot
...
- **Memory locking** — `mlockall` on Linux; per-allocation `mlock`/`VirtualLock` for
  derived key material on Unix and Windows, zeroized on drop.
- **Path-traversal protection** on the read-only skills directory: `..`, absolute paths,
  and symlinks escaping the root are all rejected.
...
- **Process-wide memory locking on Windows and macOS.** Windows has no `mlockall`
  equivalent; macOS declares it but returns `ENOSYS`. Key material is locked
  per-allocation instead, and `--classified` refuses to start where the process-wide
...
- The vault's failed-attempt counter lives in the same file it protects. An attacker who
  ...
  guessing; it is not a substitute for a TPM or secure enclave.
